...
async function searchOuter(train, first, last, line, baseDiagram, checked = [], depth = 0) {
	...
	if (first) {
		...
		const afterIndex =
			...
			:	baseDiagram.railway.stations.toReversed().findIndex((sta) => id(sta.name) === first);
		const beforeDiagram = diagrams.find((diagram) => {
			return diagram.railway.diagrams[0].trains.flat().some((d) => {
				const beforeIndex =
					...
					:	diagram.railway.stations.toReversed().findLastIndex((sta) => id(sta.name) === first);
		...
		if (beforeDiagram) {
			const before = beforeDiagram.railway.diagrams[0].trains.flat().find((d) => {
				const beforeIndex =
					d.direction === 0 ?
						beforeDiagram.railway.stations.findLastIndex((sta) => id(sta.name) === first)
					:	beforeDiagram.railway.stations.toReversed().findLastIndex((sta) => id(sta.name) === first);
			...
			const lastIndex = beforeStops.findIndex((sta) => sta.id === first && beforeDiagram.railway.name !== 'OL');
	...
	if (last) {
		...
		const beforeIndex =
			...
			:	baseDiagram.railway.stations.toReversed().findLastIndex((sta) => id(sta.name) === last);
