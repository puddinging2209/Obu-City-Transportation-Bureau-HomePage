...
export async function searchOtherStops(station, fromTime, time, train, passing, mode) {
    ...
    const froms = stops
        .filter((sta) => sta.name === name(station))
        .map((sta) => stops.indexOf(sta));
    ...
    froms.forEach((from) => {
        ...
        for (let i = from + step; i >= 0 && i < stops.length; i += step) {
            ...
            const arr =
                stop.arr +
                Number((mode === 0 && time > stop.arr) || (mode === 1 && time < stop.arr)) * 86400;
            const dep =
                stop.dep +
                Number((mode === 0 && time > stop.dep) || (mode === 1 && time < stop.dep)) * 86400;
            result.push({
                to: stop.code,
                arr,
                dep,
...
export async function searchFastestTrain(nowtime, fromsta, tosta, mode, tokkyu, visited) {
    ...
    if (nodes[fromsta].json === nodes[tosta].json) {
        ...
        let direction = 0;
        let from =
            innerstations.includes(fromsta) ?
                innerstations.indexOf(fromsta)
            :   innerstations.indexOf(fromsta.slice(0, 4));
        let to =
            innerstations.includes(tosta) ?
                innerstations.indexOf(tosta)
            :   innerstations.indexOf(tosta.slice(0, 4));
        if (from > to) {
            direction = 1;
        ...
        for (let day = 0; -2 < day && day < 2; true) {
            dia.railway.diagrams[0].trains[direction].forEach((train) => {
                if (
                    train.timetable._data[from]?.stopType === 1 &&
                    train.timetable._data[to]?.stopType === 1
                ) {
                    const arrTime =
                        adjustTime(
                            train.timetable._data[to].arrival ??
                                train.timetable._data[to].departure,
                        ) +
                        day * 86400;
                    const depTime =
                        adjustTime(
                            train.timetable._data[from].departure ??
                                train.timetable._data[from].arrival,
                        ) +
                        day * 86400;
                    ...
                    if (
                        ...
                        if (
                            ...
                            !visited.some((s) => passing.includes(s))
                        ) {
                            fastests.push({
                                train: train,
                                arr: arrTime,
        ...
        let f =
            fromStations.includes(fromsta) ?
                fromStations.indexOf(fromsta)
            :   fromStations.indexOf(fromsta.slice(0, 4));
        let t =
            toStations.includes(tosta) ?
                toStations.indexOf(tosta)
            :   toStations.indexOf(tosta.slice(0, 4));
        ...
        for (let day = 0; -2 < day && day < 2; true) {
            fromDia.railway.diagrams[0].trains.flat().forEach((fromTrain) => {
                ...
                if (
                    ...
                    fromTrain.number != '' &&
                    toDia.railway.diagrams[0].trains
                        .flat()
                        .some((d) => d.number == fromTrain.number)
                ) {
                    const toTrain = toDia.railway.diagrams[0].trains
                        .flat()
                        .find((d) => d.number == fromTrain.number);
                    ...
                    const arrTime =
                        adjustTime(
                            toTrain.timetable._data[to].arrival ??
                                toTrain.timetable._data[to].departure,
                        ) +
                        day * 86400;
                    const depTime =
                        adjustTime(
                            fromTrain.timetable._data[from].departure ??
                                fromTrain.timetable._data[from].arrival,
                        ) +
                        day * 86400;
