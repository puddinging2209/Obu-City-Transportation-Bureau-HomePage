...
export function codes(text) {
	...
	else if (stations[text]) return stations[text].code;
	else if (Object.keys(nodes).includes(text)) {
		return nodes.filter((s) => s.id === id(text)).map((s) => s.code);
	}
	return null;
}
