...
export function TimeControl() {
	...
	React.useEffect(() => {
		...
		const tick = () => {
			setDisplayTime(
				Math.floor((timeState.baseSimulationTime + ((performance.now() - timeState.startAt) * timeState.speedRate) / 1000) % (60 * 60 * 24)),
			);
		...
		<Stack spacing={1}>
			<Box spacing={1} gridTemplateColumns='40px 1fr 40px' sx={{ display: 'grid', alignItems: 'center' }}>
				...
				<Typography fontSize='1.65rem' fontFamily='monospace'>
					{Math.floor(displayTime / 3600)}:{String(Math.floor((displayTime % 3600) / 60)).padStart(2, '0')}:
			...
			<Stack direction='row' justifyContent='center' spacing={1}>
				...
				<CompactButton size='small' variant='outlined' onClick={() => setSpeed(timeState.speedRate / 5)}>
					×0.2
				</CompactButton>
				<CompactButton size='small' variant='outlined' onClick={() => setSpeed(timeState.speedRate / 2)}>
					×0.5
				</CompactButton>
				<CompactInput
					size='small'
