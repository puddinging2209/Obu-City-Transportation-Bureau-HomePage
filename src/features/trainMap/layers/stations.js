...
export async function initializeStationsLayer({ map, store }) {
	const getStationPriority = (stationData) => {
		...
		return (
			Math.max(
				...stationData.routes.map((r) => {
					return Object.values(linesData[r].stations.find((s) => s.id === stationData.id)?.types ?? {}).filter((s) => s).length;
				}),
			) *
				stationData.routes.length +
			(getDirections(stationData.id).length === 1 ? 10 : 0)
		);
	};
