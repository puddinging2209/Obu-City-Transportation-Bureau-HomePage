...
export default function TransferInput({ onSearch, loading }) {
    ...
    function toSeconds(time) {
        return (
            Number(time.format('HH')) * 3600 +
            Number(time.format('mm')) * 60 +
            Number(time.format('ss'))
        );
    }
    ...
    function queryChange(time, mode, tokkyu, allowOuterTransfer) {
        ...
        if (allowOuterTransfer !== undefined)
            params.append('allowOuterTransfer', allowOuterTransfer);
