...
function RouteStationRow({ i, line, stations, lines, onClick, selectedRouteId }) {
	...
	const transferRoutes = Object.entries(operationalMap)
		.filter(([routeId, opStations]) => {
			...
			return (
				(prevOp != null && !stations.map((s) => s.name).includes(prevOp)) || (nextOp != null && !stations.map((s) => s.name).includes(nextOp))
			);
		})
		.map(([routeId]) => routeId);
