        ...
        <>
            <Card sx={{ width: { xs: '100%', md: '70%' }, mx: 'auto', my: 4 }}>
                <CardContent>
                    ...
                    <Box sx={{ mt: 2 }}>
                        ...
                        {segments.map((seg, i) => {
                            ...
                            const isContinue =
                                i > 0 &&
                                seg.train.number === segments[i - 1]?.train.number &&
                                seg.train.number !== '';
                            ...
                            return seg.line.map((line, j) => {
                                ...
                                const isInnerContinueNext =
                                    j !== seg.line.length - 1 || isContinueNext;
                                const isSameLineName =
                                    j > 0 && lines[line]?.show === lines[seg.line[j - 1]]?.show;
                                return (
                ...
                <TrainStopsDialog
                    ...
                    emphasized={[
                        `${pushed?.from},${pushed?.depTime}`,
                        `${pushed?.to},${pushed?.arrTime}`,
                    ]}
                />
            </Card>
        </>
...
function StationBox({
    ...
    depTime,
    StationId,
    disableArrTime = false,
    disableDepTime = false,
}) {
    ...
    return (
        <Box sx={{ width: '100%', display: 'flex', borderRadius: 1, p: 1, gap: 1 }} bgcolor='#DDD'>
            <Box sx={{ flex: '0 0 42px', textAlign: 'center' }}>
                <Typography variant='body1'>
                    {disableArrTime ? '出発' : toTimeString(arrTime)}
                </Typography>
                <Typography variant='body1'>
                    {disableDepTime ? '到着' : toTimeString(depTime)}
                </Typography>
            </Box>
            <Box
                sx={{
